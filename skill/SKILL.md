...
On multi-page work, enforce **minimum progress per turn** (at least one planned batch merged or one structural unit advanced) before voluntary yield; see `references/goal-mode.md`. Use scale-aware compile cadence (chapter-level on large projects). Before ending a turn with unfinished work, leave a concrete Next action in `conversion-state.md` (batch id / pages / file when possible).
