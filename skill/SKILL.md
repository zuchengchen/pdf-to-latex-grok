...
## Hard Continuity Rule (Non-Negotiable)

**Run-to-completion is the default for every full conversion.** When the user asks to convert, rebuild, resume, or refine a PDF into LaTeX, finish the entire pipeline in this session: triage → scaffold → evidence → all planned batches → merge → integration → refinement → required gates → terminal outcome. Do not stop mid-work to ask whether to continue.

**Forbidden stop prompts (never emit these as a reason to pause):** 是否继续, 要不要继续, 继续吗, 需要我继续吗, Shall I continue?, Continue?, say continue, reply continue, type 继续, or any equivalent “wait for user approval to proceed with ordinary reconstruction.”

Do not end a turn after only one batch, one chapter, a progress summary, or the first successful compile while open work remains. Keep looping automatically. Durable `conversion-state.md` is for forced host stops and later resume—not for voluntary mid-pipeline chat handoffs. Details: `references/goal-mode.md`.
...
**Never ask the user whether to continue ordinary reconstruction.** Progress questions are not user-decision boundaries. Ask only when a true authorization boundary needs a decision: overwrite risk, source replacement, unsafe build capability, material approximation, delivery downgrade, a conflicting unrelated Goal, an unreadable required region that blocks the chosen delivery contract, or another real authorization boundary. Goal startup alone is not such a boundary; never block on Goal startup and do not ask solely whether to enable Goal mode. Batch boundaries, chapter boundaries, midpoint review, and the first successful compile are never such boundaries. Otherwise continue to the selected completion standard without pausing.

**Run-to-completion (hard default).** For full `convert`, broad `resume` / `refine`, and other multi-batch work, keep working in the same session until the project reaches a terminal outcome (`complete`, `blocked`, or user-approved `downgraded`). Do not stop after one batch, one chapter, the first successful compile, a progress narration, or a midpoint checkpoint and wait for the user to say 继续 / continue / next / 是否继续. Never prompt the user to type those words just to proceed. See `references/goal-mode.md` for the full loop: dispatch remaining batches (prefer concurrent `spawn_subagent` workers), merge, integrate, compile at scale-aware cadence, run refinement and required gates, then report. Yield only for a true user-decision boundary, a hard environment failure, or a genuine host runtime limit; if forced to stop mid-work, write a concrete Next action in `conversion-state.md` (batch id / pages / file) so a later resume can finish without re-triage.
